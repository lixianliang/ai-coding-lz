...
func (db *Database) UpdateDocumentSummaryImageURL(ctx context.Context, id string, imageURL string) error {
	rowsAffected, err := gorm.G[Document](db.db).Where("id = ?", id).Update(ctx, "summary_image_url", imageURL)
	if err != nil {
		return err
	}
	if rowsAffected == 0 {
